...
float refine(const std::string& filename_xyz, const std::string& filename_uv,
  ...
  if(logfile != 0){
    *logfile << tracking_offset_time << ";" << (avg_quality-0.999)*1000.0 << ";" << 0.0 << ";" << 0.0 << std::endl;
  }
  ...
  for(unsigned rs = 0; rs < 15; ++rs){
    ...
    avg_quality = sampleQuality(filename_xyz, filename_uv, cs_sweep, cfg, cb,
				tracking_offset_time, color_offset_time, optimization_stride, idwneighbours, using_nni, basefilename,
				sensor, eye_d_to_world);
    if(logfile != 0){
      *logfile << tracking_offset_time << ";" << (avg_quality-0.999)*1000.0 << ";" << 0.0 << ";" << 0.0 << std::endl;
...
float optimizeParabelFitting(const std::string& filename_xyz, const std::string& filename_uv,
  ...
  if(logfile != 0){
    *logfile << "optimizeParabelFitting" << std::endl;
    *logfile << "tracking_offset_time;avg_quality;best_tracking_offset_time;best_avg_quality" << std::endl;
  }
  ...
  if(logfile != 0){ // (avg_quality - 0.999)*1000.0
    printParabolaToLog(x1, x2, 0.001,
		       x1, x2, x3,
...
float optimizeBruteForce(const std::string& filename_xyz, const std::string& filename_uv,
  ...
  for(float tracking_offset_time = tracking_offset_time_min;
    ...
    if(logfile != 0){ // (avg_quality - 0.999)*1000.0
      *logfile << tracking_offset_time << ";" << std::setprecision(10) << (avg_quality - 0.999)*1000.0 << ";" << best_tracking_offset_time << ";" << std::setprecision(10) << (best_avg_quality - 0.999)*1000.0 << std::endl;
    }
...
float optimizeParabelFitting3D(const std::string& filename_xyz, const std::string& filename_uv,
  ...
  if(logfile != 0){
    *logfile << "optimizeParabelFitting3D" << std::endl;
    *logfile << "tracking_offset_time;avg_quality;best_tracking_offset_time;best_avg_quality" << std::endl;
  }
...
float optimizeParabelFitting2D(const std::string& filename_xyz, const std::string& filename_uv,
  ...
  if(logfile != 0){
    *logfile << "optimizeParabelFitting2D" << std::endl;
    *logfile << "color_offset_time;avg_2D_error;best_color_offset_time;best_avg_2D_error" << std::endl;
  }
