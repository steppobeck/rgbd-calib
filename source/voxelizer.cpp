...
int main(int argc, char* argv[]){
  ...
  p.addOpt("f",1,"feedbackport", "specify port of receiving socket (for receiving from client)");
  ...
  if(p.isOptSet("f")){
    feedback = new udpconnection;
    feedback->open_receiving_socket(p.getOptsInt("f")[0]);
  }
  ...
  while(frame_num < num_frames){
    ...
    if(feedback != 0){
      float voxelsize_want = 0.008;
      size_t bytes_received = feedback->recv(&voxelsize_want, sizeof(voxelsize_want), false /*do no block*/);
      if(bytes_received == sizeof(voxelsize_want)){
	std::cout << "received new voxelsize_want: " << voxelsize_want << std::endl;
	voxelsize_inv = 1.0f/voxelsize_want;
    ...
    */
	...
	++packet_number;
      ...
      if(0 == voxel_number){
	if(feedback != 0){
	  double voxelsize_sent = 1.0f/voxelsize_inv;
	  memcpy(&buff[buff_index], &voxelsize_sent, sizeof(voxelsize_sent));
