...
const cv::Size image_size_rgb(1280,1080);
const cv::Size image_size_depth_ir(512,424);
                            ...
                            std::vector< DepthCalibrationPoint >& depth_values)
{
    ...
    double error = calibrateCamera(pattern_points, good_corners, global::image_size_depth_ir,
...
void calibrate_kinect_rgb(std::vector< std::vector<Point2f> >& stereo_corners)
{
    ...
    std::vector<Mat> rvecs, tvecs;
    double error = calibrateCamera(pattern_points, good_corners, global::image_size_rgb,
...
void writeNestkMatrix()
{
    ...
    cv::Mat1i size_matrix_rgb(1,2);
    size_matrix_rgb(0,0) = global::image_size_rgb.width;
    size_matrix_rgb(0,1) = global::image_size_rgb.height;
    cv::Mat1i size_matrix_depth(1,2);
    size_matrix_depth(0,0) = global::image_size_depth_ir.width;
    size_matrix_depth(0,1) = global::image_size_depth_ir.height;
    writeMatrix(output_file, "rgb_size", size_matrix_rgb);
    writeMatrix(output_file, "raw_rgb_size", size_matrix_rgb);
    writeMatrix(output_file, "depth_size", size_matrix_depth);
    writeMatrix(output_file, "raw_depth_size", size_matrix_depth);
