...
SweepSampler::addBoardSamples(const glm::mat4& cb_transform, const ChessboardViewIR* corners_ir, const ChessboardViewRGB* corners_rgb){
  ...
  for(unsigned idx = 0; idx < (CB_WIDTH * CB_HEIGHT); ++idx){
    ...
    if(0.0 == corners_ir->quality[idx] || 0.0 == corners_rgb->quality[idx]){
      std::cout << "INFO: corner quality of either RGB or IR is 0 ... skipping this corner! " << std::endl;
      continue;
    }
    ...
    // new
    glm::vec4 pos_realH = (cb_transform * m_cb->pose_offset) * glm::vec4(m_cb->points_local[idx].x, m_cb->points_local[idx].y, m_cb->points_local[idx].z, 1.0f);
